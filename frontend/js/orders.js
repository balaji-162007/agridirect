...
if (document.getElementById('checkoutLayout')) {
  ...
  window.placeOrder = async function() {
    ...
    try {
      if (_payment === 'cod') {
        const res = await apiFetch('/orders', 'POST', payload);
        if (res) _showSuccess(res.order_id);
      } else {
        /* Razorpay */
        const rzp = await apiFetch('/payments/create-order', 'POST', { amount: total, currency: 'INR' });
        if (!rzp) throw new Error('Could not create payment order');
        ...
        const opts = {
          ...
          handler: async function(r) {
            ...
            const res = await apiFetch('/payments/verify', 'POST', vPayload);
            if (res) _showSuccess(res.order_id);
          },
